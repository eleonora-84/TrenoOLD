...
import builder.TN.TNLocomotiva;
import dao.LocomotivaDao;
import vagoni.Locomotiva;

public class LocomotivaTNDaoImpl implements LocomotivaDao {
	
	private static LocomotivaTNDaoImpl instance = null;
	
	private LocomotivaTNDaoImpl() {
	
	}
	
	public static LocomotivaTNDaoImpl getInstance() {
		if (instance == null) 
			instance = new LocomotivaTNDaoImpl();
		return instance;
	}
	...
	public int add() {
		...
		try {
			tx = session.beginTransaction();
			Locomotiva l = new TNLocomotiva();
			locomotivaID = (Integer) session.save(l);
